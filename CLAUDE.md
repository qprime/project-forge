...
Forge produces hybrid runtime projects but is not itself one of its own consumers. Skills, invariants, and conventions for forge are authored directly in this repo, not generated by forge from its own baseline.
...
Forge does not consume its own pipeline. The only skill at the project layer is `/spec`, authored directly at `.claude/commands/spec.md`. Add forge-specific skills there as needed; do not introduce a `.forge/manifest.yaml` to generate them.
