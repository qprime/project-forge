...
3. **Project content** (project)
   ...
   - lives at `<project>/<skills_dir>/<skill>.custom.md`, where `skills_dir` is declared in the project's manifest. For forge that's `.claude/commands/` (#10). Pinned in #10.
   - Authoring prompts (the LLM-facing prompt that generates a project's `.custom.md` file) live in `skills/custom/<skill>.prompt.md` in the **baseline**, not in the project. Authoring-prompt and project-layer-contribution are different artifacts at different paths.
...
- Project-layer contribution location → #10 (`<project>/<skills_dir>/<skill>.custom.md`; `skills_dir` per manifest; for forge `.claude/commands/`)
